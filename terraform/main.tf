...
# Target Groups
resource "aws_lb_target_group" "project1" {
  name     = "project1-tg"
  port     = 80
  protocol = "HTTP"
  vpc_id   = aws_vpc.main.id
...
resource "aws_lb_target_group_attachment" "project1" {
  target_group_arn = aws_lb_target_group.project1.arn
...
resource "aws_lb_listener" "http" {
  ...
  default_action {
    ...
    forward {
      target_group {
        arn    = aws_lb_target_group.project1.arn
        weight = 50
      }
      target_group {
        arn    = aws_lb_target_group.project2.arn
        weight = 50
      }
    }
  }
...
resource "aws_lb_listener_rule" "project1" {
  ...
  condition {
    host_header {
      values = [var.host_header_project1]
    }
  }
}
...
resource "aws_lb_listener_rule" "project2" {
  ...
  condition {
    host_header {
      values = [var.host_header_project2]
    }
  }
}
